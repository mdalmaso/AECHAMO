...
vap_wallsink_on = 1;
vap_wallsink  = [0; 1/9; 1/90; 1/900; 1/9000; 1/90000];

Dp_min = -9;
Dp_max = -6;
sections = 40;

tvect = 0:600:24*3600; % time starts and stops at 00:00
...
gas_source(:,:,2) = [tvect' [0.3*2e-16*60e-9*2.6908e19*100e-9*2.6908e19*exp(-2e-16.*60e-9.*2.6908e19.*tvect)]']; % 11h no event, 2h event, 11h no event
gas_source(:,:,1) = [tvect' [0.3*9e-17*30e-9*2.6908e19*0.25*1e-9*2.6908e19.*exp(-9e-17*30e-9*2.6908e19.*tvect)]']; % 11h no event, 2h event, 11h no event
Cvap0 = 0;
