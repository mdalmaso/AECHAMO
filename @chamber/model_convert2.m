function [ out_struct ] = model_convert2(obj, t, Y)
% MODEL_CONVERT creates a structure of results calculated by simulation.
% 
% This model_convert2 is used, when method 'moving_center_beta' is used.
% Otherwise use function model_convert.
% 
...
% very narrow.
Dp0_dense = logspace(initials.Dp_min, initials.Dp_max, initials.output_sections);

% Preallocate the variables for the loop.
Ntot=zeros(1,length(t));
dN=zeros(length(t),length(Dp0_dense));
...
for i = 1:length(t)
    real_indices = find(~isnan(Y(i,nSec+2:2*nSec+1)));
    
    Dpi = Y(i,nSec+1+real_indices);
    Ni = Y(i,1+real_indices);
    
    dNi = obj.N_to_dlog(Dpi,Ni);
    
    Ntot(i) = sum(Y(i,1+real_indices));
    
    dist_original(i+1,2) = Ntot(i);
    dist_original(i+1,3:nSec+2) = interp1(log10(Dpi),dNi,log10(Dp0),'linear',0);
    dist_original(i+1,nSec+3:end) = Dp0;
    
    
    dN(i,:) = interp1(log10(Dpi),dNi,log10(Dp0_dense),'linear',0);
...
dist(1,3:end) = Dp0_dense;     % The first row of dist tells the Dp0s of distribution.
