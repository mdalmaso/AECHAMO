...
% Initial conditions:
y0 = [initials.Cvap0 N Dp AE_Wall AE_dilu Vap_dilu Dp2];
...
% Close waitbar
% close(h);
Y2 = Y(:,1:2*initials.sections+4);
% This makes a handy structure of the results:
obj.output_data = obj.model_convert(t,Y2);
...
% Condensation
distr0 = [y(nSec+2:2*nSec+1)'; % distribution [Dp1 Dp2 ...
              y(2:nSec+1)'];   %               N1  N2  ...]
          
distr_temp = [y(2*nSec+5:3*nSec+4)';
                y(2:nSec+1)'];

Kn = (2.*lambda)./distr0(1,:); % Knudsen number
%     Kn = lambda./y(nSec+1+i);
betam = (Kn+1)./((0.377.*Kn)+1+(4/(3.*alfa)).*(Kn.^2)+(4/(3.*alfa)).*Kn);

% the flux of molecules to the particle phase
I = 2.*pi.*distr0(1,:).*1e2.*diffu.*(y(1)-Csat).*betam; %1/s

dy(2*nSec+5:3*nSec+4)=(2.*mv.*I)./(pi.*rool.*distr0(1,:).^2.*NA.*1e6);




    % Interpolate this moved distribution to the original distribution's
    % sections:
    distr_temp(2,:) = interp1(distr_temp(1,:),distr_temp(2,:),distr0(1,:),'linear',0);
    % And replace also the Dp:s with original distribution's Dp:s:
    distr_temp(1,:) = distr0(1,:);
    
    % Get the total number of particles in this new distribution:
    N_tot_new = sum(distr_temp(2,:));
    % And the total number of particles in the original distribution:
    N_tot = sum(distr0(2,:));
    
    delta_N = N_tot-N_tot_new; % Difference between the original and new
                            % distributions particle concentration.
    % Add the missing particles to the new distribution's sections
    % weighting by the amount of particles in the section:
    distr_temp(2,:) = distr_temp(2,:)+distr_temp(2,:).*delta_N./N_tot_new;
...
for i = 1:nSec,
    ...
    % Condensation
    % Calculate the difference between the original distribution and the
    % distribution grown by condensation. This change is added to dy.
    delta_N = distr_temp(2,i)-distr0(2,i);
    dy(1+i)=dy(1+i)+delta_N;
    % end condensation
    ...
    % zero so that y(1) stays constant.
