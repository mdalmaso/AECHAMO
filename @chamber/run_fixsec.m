...
Kn = (2.*lambda)./distr_temp(1,:); % Knudsen number
%     Kn = lambda./y(nSec+1+i);
betam = (Kn+1)./((0.377.*Kn)+1+(4/(3.*alfa)).*(Kn.^2)+(4/(3.*alfa)).*Kn);

% the flux of molecules to the particle phase
I = 2.*pi.*distr_temp(1,:).*1e2.*diffu.*(y(1)-Csat).*betam; %1/s

delta_t = t-t2

if(delta_t > 0)
    % Move the center of each section according to condensation:
    distr_temp(1,:) = distr_temp(1,:)+(2.*mv.*I)./(pi.*rool.*distr_temp(1,:).^2.*NA.*1e6).*delta_t;
