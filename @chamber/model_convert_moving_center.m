...
function[out_struct] = model_convert_moving_center(obj,t,Y)
...
for i = 1:length(t),
    ...
    Ni_temp = Y(i,2:nSec+1);
    Dpi_temp = Y(i,nSec+2:(2*nSec+1));
    for j=1:length(Dpi_temp)/2
        Ni(j)=Ni_temp(2*j) + Ni_temp(2*j-1);
        if(Ni(j) > 0)
            Dpi(j) = ((Ni_temp(2*j)*Dpi_temp(2*j)^3 + Ni_temp(2*j-1)*Dpi_temp(2*j-1)^3)/Ni(j))^(1/3);
        else
            Dpi(j) = (Dpi_temp(2*j)^3 + Dpi_temp(2*j-1)^3)^(1/3);
        end
    end
