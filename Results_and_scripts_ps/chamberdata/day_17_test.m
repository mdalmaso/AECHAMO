...
part_source(:,:,1) = [tvect' [zeros(858,1);23.5.*ones(21,1);zeros(length(tvect)-858-21,1)], 10e-9.*ones(length(tvect),1)];
part_source(:,:,2) = [tvect' [zeros(858,1);zeros(21,1);8.*ones(14,1);0.35.*ones(77,1);1.3.*ones(16,1);3.7.*ones(66,1);3.1.*ones(131,1);zeros(length(tvect)-858-21-15-76-16-66-131,1)], 20e-9.*ones(length(tvect),1)];
...
gas_source = [tvect' [interp1(timehours.*3600,Q(begin:last),tvect(1:1182),'linear',0),zeros(1,length(tvect)-1182)]'];

dilu_coeff = [tvect' [interp1(timehours.*3600,dilu(begin:last),tvect).*0.88]'];
clear begin;
