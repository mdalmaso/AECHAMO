...
% part_source(:,:,1) = [tvect' [zeros(863,1);3.8.*ones(39,1);zeros(length(tvect)-863-39,1)], 10e-9.*ones(length(tvect),1)];
% part_source(:,:,2) = [tvect' [zeros(863,1);zeros(39,1);2.2.*ones(30,1);0.7.*ones(15,1);1.75.*ones(57,1);2.3.*ones(176,1);zeros(length(tvect)-863-39-30-15-57-176,1)], 18e-9.*ones(length(tvect),1)];

part_source(:,:,1) = [tvect' [3.55.*ones(39,1);zeros(length(tvect)-39,1)], 10e-9.*ones(length(tvect),1)];
part_source(:,:,2) = [tvect' [zeros(39,1);1.92.*ones(30,1);0.9.*ones(14,1);1.55.*ones(56,1);2.2.*ones(178,1);zeros(length(tvect)-39-30-15-55-178,1)], 18e-9.*ones(length(tvect),1)];
...
gas_source = [tvect' [interp1(timehours.*3600,Q1(begin:end),tvect(1:317))+interp1(timehours.*3600,Q2(begin:end),tvect(1:317)),interp1(timehours.*3600,Q2(begin:end),tvect(318:end))]'];

dilu_coeff = [tvect' [interp1(timehours.*3600,dilu(begin:end),tvect)]'];
clear begin;


gas_source(find(isnan(gas_source)))=nanmean(Q1)+nanmean(Q2);
