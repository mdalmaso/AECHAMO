...
part_source(:,:,1) = [tvect' [6.3.*ones(37,1);zeros(length(tvect)-37,1)], 10e-9.*ones(length(tvect),1)];
part_source(:,:,2) = [tvect' [zeros(35,1);1.5.*ones(95,1);2.7.*ones(190,1);zeros(length(tvect)-35-95-190,1)], 18e-9.*ones(length(tvect),1)];
...
% gas_source = [tvect' [interp1(timehours.*3600,Q(begin:end),tvect(1:328)),zeros(1,length(tvect)-328)]'];
gas_source = [tvect' [interp1(source_time,0.38.*Q_MTOH,tvect(1:end))]'];
