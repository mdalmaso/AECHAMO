...
part_source(:,:,1) = [tvect' [zeros(858,1);24.0.*ones(21,1);zeros(length(tvect)-858-21,1)], 10e-9.*ones(length(tvect),1)];
part_source(:,:,2) = [tvect' [zeros(858,1);zeros(21,1);10.*ones(11,1);1.15.*ones(98,1);3.97.*ones(64,1);3.4.*ones(133,1);zeros(length(tvect)-858-21-11-98-64-133,1)], 20e-9.*ones(length(tvect),1)];
...
gas_source = [tvect' [interp1(timehours.*3600,Q(begin:last),tvect(1:1185),'linear',0),zeros(1,length(tvect)-1185)]'];
