...
part_source(:,:,1) = [tvect' [zeros(223,1);3.0.*ones(68,1);zeros(length(tvect)-223-68,1)], 10e-9.*ones(length(tvect),1)];
part_source(:,:,2) = [tvect' [zeros(223,1);zeros(68,1);2.25.*ones(40,1);1.35.*ones(35,1);1.65.*ones(95,1);zeros(length(tvect)-223-68-40-35-95,1)], 18e-9.*ones(length(tvect),1)];
...
% gas_source = [tvect' [interp1(timehours.*3600,Q(begin:end),tvect(1:317)),zeros(1,length(tvect)-317)]'];
gas_source = [tvect' [interp1(source_time,0.34.*Q_MTOH+0.0.*Q_MTO3,tvect(1:end))]'];
