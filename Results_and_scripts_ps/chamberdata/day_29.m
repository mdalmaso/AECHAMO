...
part_source(:,:,1) = [tvect' [zeros(20,1);11.5.*ones(30,1);zeros(length(tvect)-20-30,1)], 10e-9.*ones(length(tvect),1)];
part_source(:,:,2) = [tvect' [zeros(20,1);zeros(30,1);4.5.*ones(18,1);1.65.*ones(62,1);2.3.*ones(60,1);0.6.*ones(98,1);1.4.*ones(132,1);zeros(length(tvect)-20-30-18-62-60-98-132,1)], 18e-9.*ones(length(tvect),1)];
...
% gas_source = [tvect' [interp1(timehours.*3600,Q(begin:end),tvect(1:399)),zeros(1,length(tvect)-399)]'];
gas_source = [tvect' [interp1(source_time,0.25.*Q_MTOH+0.05.*Q_MTO3,tvect(1:end),'linear',0)]'];
